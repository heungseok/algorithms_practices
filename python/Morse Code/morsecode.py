...
def is_validated_english_sentence(user_input):
    ...
    # pre-remove specific special characters
    new_user_input = re.sub("[.,!?]", "", user_input);

    # check if user input contains any number
    if any(char.isdigit() for char in new_user_input):
        result = False
    # check if user input contains special character using regular expression
    elif re.search("[^a-zA-Z0-9 ]+", new_user_input):
        result = False
    # check if user input consist with only space
    elif new_user_input.isspace():
        result = False
    # check if user input has no alphabet except special character (.,!?)
    elif len(new_user_input) == 0:
        result = False
    else:
        result = True
...
def is_validated_morse_code(user_input):
    ...
    strip_input = user_input.split(" ")
    temp_res = True
    for code in strip_input:
...
def encoding_character(english_character):
    ...
    # pre-defined morsecode는 대문자 알파벳만을 key 값으로 가지므로 capitalize
    result = morse_code_dict[english_character.capitalize()]
...
def encoding_sentence(english_sentence):
    ...
    splited_sentence = cleaned_sentence.split(" ")
...
def main():
    ...
    while True:
        ...
        if is_help_command(user_input):
            ...
            encoding_character(user_input)
        elif is_validated_english_sentence(user_input):
            decoding_character(user_input)
        elif user_input == "0":
            break
